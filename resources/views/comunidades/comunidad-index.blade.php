        ...
        <div class="container py-4">
            ...
            <div
                ...
                <div class="mb-3 mb-md-0">
                    ...
                    <p class="mb-0 text-secondary text-sm">Aquí puedes gestionar y generar reportes de las comunidades.
                    </p>
                </div>

                {{-- Botón Nueva Comunidad con Modal --}}
                <button type="button" class="btn btn-success px-4" style="height: fit-content;" data-bs-toggle="modal"
                    data-bs-target="#createComunidadModal">
                ...
                <div class="d-flex flex-column flex-md-row justify-content-between align-items-center mb-4 gap-3">
                    ...
                    <div class="dropdown w-100 w-md-auto">
                        <button class="btn text-white dropdown-toggle mb-0 px-4 w-100 w-md-auto"
                            ...
                            id="dropdownGenerate" data-bs-toggle="dropdown" aria-expanded="false">
                            Generar Reporte
                        </button>
                            ...
                            <table class="table table-hover table-bordered align-middle text-center mb-0">
                                ...
                                <tbody id="comunidades-table-body">
                                    @forelse ($comunidades as $com)
                                        <tr data-name="{{ Str::lower($com->nombre) }}"
                                            data-parroquia-id="{{ $com->parroquia_id }}">
                                            <td><input type="checkbox" name="ids[]" value="{{ $com->id }}"
                                                    style="cursor: pointer;"></td>
                                            <td class="fw-bold text-secondary">
                                                {{ $comunidades->firstItem() + $loop->index }}</td>
                                            <td class="fw-bold text-dark">{{ $com->codigo_unico }}</td>
                                            <td class="text-start ps-4">{{ $com->nombre }}</td>
                                            <td><span
                                                    class="badge border text-dark bg-light">{{ $com->parroquia->nombre }}</span>
                                            </td>
                                            <td><span
                                                    class="badge border text-dark bg-light">{{ $com->parroquia->canton->nombre }}</span>
                                            </td>
                                            <td>
                                                <button type="button" class="btn btn-sm btn-warning mb-0 me-1"
                                                    data-bs-toggle="modal" data-bs-target="#editComunidadModal"
                                                    data-id="{{ $com->id }}" data-nombre="{{ $com->nombre }}"
                ...
                <div class="modal-content">
                    ...
                    <form action="{{ route('comunidades.store') }}" method="POST">
                        ...
                        <div class="modal-body">
                            <div class="alert alert-info py-2 mb-3 text-xs"><i class="fas fa-info-circle me-1"></i>
                                Código automático (Ej: COM001).</div>
                            <div class="mb-3">
                                <label class="form-label fw-bold">Parroquia</label>
                                <select name="parroquia_id" class="form-select" required>
                                    <option value="">Seleccionar...</option>
                                    @foreach($parroquias as $p) <option value="{{ $p->id }}">{{ $p->nombre }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="mb-3">
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <form id="editComunidadForm" method="POST">
                        @csrf @method('PUT')
                        <div class="modal-body">
                            <div class="mb-3"><label class="form-label fw-bold text-muted">Código</label><input
                                    type="text" id="editCodigo" class="form-control bg-light" readonly></div>
                            <div class="mb-3">
                                <label class="form-label fw-bold">Parroquia</label>
                            ...
                            <div class="mb-3"><label class="form-label fw-bold">Nombre</label><input type="text"
                                    name="nombre" id="editNombre" class="form-control" required></div>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
